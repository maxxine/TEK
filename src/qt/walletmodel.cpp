...
{
    CtekcoinAddress addressParsed(address.toStdString());
    return addressParsed.IsValid();
}

WalletModel::SendCoinsReturn WalletModel::sendCoins(const QList<SendCoinsRecipient> &recipients, int nSplitBlock, const CCoinControl *coinControl) //presstab
{
    qint64 total = 0;
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            scriptPubKey.SetDestination(CtekcoinAddress(rcp.address.toStdString()).Get());
        ...
        bool fCreated = wallet->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, nSplitBlock, coinControl); //presstab

        if(!fCreated)
        {
            if((total + nFeeRequired) > nBalance) //presstab
            {
...
void WalletModel::getStakeWeightFromValue(const int64& nTime, const int64& nValue, uint64& nWeight) //stake weight addition to code presstab
{
...
{
    OutputDebugStringF("NotifyAddressBookChanged %s %s isMine=%i status=%i\n", CtekcoinAddress(address).ToString().c_str(), label.c_str(), isMine, status);
    QMetaObject::invokeMethod(walletmodel, "updateAddressBook", Qt::QueuedConnection,
                              Q_ARG(QString, QString::fromStdString(CtekcoinAddress(address).ToString())),
...
//presstab add everything below here
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        mapCoins[CtekcoinAddress(address).ToString().c_str()].push_back(out);
    }
}
...
bool WalletModel::isMine(const CtekcoinAddress &address)
{
