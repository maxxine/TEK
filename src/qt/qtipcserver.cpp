...
static bool ipcScanCmd(int argc, char *argv[], bool fRelay)
{
    ...
    {
        if (boost::algorithm::istarts_with(argv[i], "tekcoin:"))
        {
            const char *strURI = argv[i];
            try {
                boost::interprocess::message_queue mq(boost::interprocess::open_only, tekcoinURI_QUEUE_NAME);
...
static void ipcThread2(void* pArg)
{
    ...
    message_queue::remove(tekcoinURI_QUEUE_NAME);
...
void ipcInit(int argc, char *argv[])
{
    ...
    try {
        mq = new message_queue(open_or_create, tekcoinURI_QUEUE_NAME, 2, MAX_URI_LENGTH);

        // Make sure we don't lose any tekcoin: URIs
        for (int i = 0; i < 2; i++)
        {
        ...
        // Make sure only one tekcoin instance is listening
        message_queue::remove(tekcoinURI_QUEUE_NAME);
        delete mq;
