...
//#include "base58.h" presstab
#include "coincontrol.h" //presstab
#include "coincontroldialog.h" //presstab
#include <QMessageBox>
#include <QLocale>
#include <QTextDocument>
#include <QScrollBar>
#include <QClipboard> //presstab
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    //QString txcomment = ui->editTxComment->text(); presstab
    ...
    WalletModel::SendCoinsReturn sendstatus; //presstab s
    if (!model->getOptionsModel() || !model->getOptionsModel()->getCoinControlFeatures())
        sendstatus = model->sendCoins(recipients);
    else
        sendstatus = model->sendCoins(recipients, CoinControlDialog::coinControl); //presstab e
    ...
    case WalletModel::OK:
        accept();
		CoinControlDialog::coinControl->UnSelectAll(); //presstab
		coinControlUpdateLabels(); //presstab
...
void SendCoinsDialog::clear() //presstab replaced to remove txcomment
{
...
SendCoinsEntry *SendCoinsDialog::addEntry()
{
	...
	connect(entry, SIGNAL(payAmountChanged()), this, SLOT(coinControlUpdateLabels())); //presstab
...
QWidget *SendCoinsDialog::setupTabChain(QWidget *prev) //presstab replaced to remove txcomment
{
    for(int i = 0; i < ui->entries->count(); ++i)
    {
        SendCoinsEntry *entry = qobject_cast<SendCoinsEntry*>(ui->entries->itemAt(i)->widget());
...
//presstab everything below here added
